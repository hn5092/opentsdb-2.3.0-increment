...
public final class TSDB {
  ...
  final Deferred<Long> increment(final byte[] key, final byte[] qualifier, final byte[] value, final long amount) {
    return client.bufferAtomicIncrement(new AtomicIncrementRequest(table, key, FAMILY, qualifier, amount));
  }


  public Deferred<Long> addIncrementPoint(final String metric,
                                            final long timestamp,
                                            final byte[] value,
                                            final Map<String, String> tags,
                                            final long amount) {
    final short flags = 7; //increment全部都是long
    return addIncrementPointInternal(metric, timestamp, value, flags, tags, amount);
  }


  private Deferred<Long> addIncrementPointInternal(final String metric,
                                                     final long timestamp,
                                                     final byte[] value,
                                                     final short flags,
                                                     final Map<String, String> tags,
    ...
    /** Callback executed for chaining filter calls to see if the value
     * should be written or not. */
    final class WriteCB implements Callback<Deferred<Long>, Boolean> {
      @Override
      public Deferred<Long> call(final Boolean allowed) throws Exception {
        if (!allowed) {
          rejected_dps.incrementAndGet();
          return Deferred.fromResult(null);
        }
        ...
        Deferred<Long> result =  increment(row, qualifier, value, amount);
//        if (config.enable_appends()) {
//          final AppendDataPoints kv = new AppendDataPoints(qualifier, value);
//          final AppendRequest point = new AppendRequest(table, row, FAMILY,
//              AppendDataPoints.APPEND_COLUMN_QUALIFIER, kv.getBytes());
